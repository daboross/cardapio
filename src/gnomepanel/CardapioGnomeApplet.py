...
class CardapioGnomeApplet(CardapioAppletInterface):
    ...
    def _on_panel_size_changed(self, widget, allocation):
        ...
        glib.timeout_add(CardapioGnomeApplet.PANEL_SIZE_CHANGE_IGNORE_INTERVAL,
                         self._on_panel_size_change_done)  # added this to avoid an infinite loop
    ...
    def _load_settings(self):
        ...
        if self.applet_icon:
            button_icon_pixbuf = self.icon_helper.get_icon_pixbuf(self.applet_icon,
                                                                  self._get_best_icon_size_for_panel(), 'start-here')
