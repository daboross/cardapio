...
if gtk.ver < (2, 14, 0):
    fatal_error('Fatal error loading Cardapio',
...
class Cardapio(dbus.service.Object):
    ...
    def __init__(self, show=False, panel_applet=None, debug=False):
        ...
        if show == Constants.SHOW_NEAR_MOUSE:
            self.show_hide_near_mouse()
    ...
    def _get_applet(self, applet):
        ...
        if applet is None:
            try:
                applet = CardapioSimpleDbusApplet(self._bus)
            except:
                applet = CardapioStandAlone()
    ...
    def _create_xdg_folders(self):
        ...
        if not os.path.exists(self._config_folder_path):
            ...
            fatal_error('Error creating config folder!',
                        'Cannot create folder "%s" because a file with that name already exists!' % self._config_folder_path)
            self._quit()
        ...
        if not os.path.exists(self._cache_folder_path):
            ...
            fatal_error('Error creating cache folder!',
    ...
    def _process_query(self, ignore_if_unchanged):
        ...
        if len(text) == 0:
            ...
            self._view.set_all_sections_sidebar_button_sensitive(True, self._in_system_menu_mode)
        ...
        if not search_inside:
            ...
            # if pushing into a folder
                ...
                if first_app_info is not None:
                    if selected_app_info is not None:
                        app_info = selected_app_info
                    else:
                        app_info = first_app_info
    ...
    def _file_looper_generator(self, matches, path, ignore_hidden):
        ...
        for filename in matches:
            ...
            if count > limit:
                ...
                if page == 5:
                    self._add_app_button(_('Open this folder'), 'system-file-manager', self._view.SUBFOLDERS_SECTION,
                                         'xdg', path, _('Show additional search results in a file browser'), None)
                    yield count

                load_more_button = self._add_app_button(_('Load additional results'), 'add',
                                                        self._view.SUBFOLDERS_SECTION, 'special',
                                                        self._load_more_subfolder_results,
                                                        _('Show additional search results'), None)
                yield count
    ...
    def _schedule_search_with_specific_plugin(self, text, delay_type=None, specific_plugin=None):
        ...
        if delay_type is None:
            ...
            self._search_timer_local = glib.timeout_add(timer_delay, self._search_with_specific_plugin, text,
    ...
    def _plugin_handle_search_result_synchronized(self, plugin, results, original_query):
        ...
        for result in results:
            ...
            button = self._add_app_button(result['name'], icon_name, plugin.section, result['type'], result['command'],
                                          result['tooltip'], None)
    ...
    def _restore_dimensions(self, x=None, y=None, force_anchor_right=False, force_anchor_bottom=False):
        ...
        x, y, anchor_right, anchor_bottom = self._get_coordinates_inside_screen(x, y, force_anchor_right,
                                                                                force_anchor_bottom)
    ...
    def _fill_system_list(self):
        ...
        section, dummy = self.add_section(_('Uncategorized'), 'applications-other',
                                          tooltip=_('Other configuration tools'), hidden_when_no_query=False,
                                          system_menu=True)
    ...
    def _fill_system_places_list(self):
        ...
        for mount in self._volume_monitor.get_mounts():
            ...
            try:
                command = str(volume.get_mount().get_root().get_uri())
            except:
                command = ''
        ...
        self._add_app_button(_('Network'), 'network', section, 'xdg', 'network://',
                             _('Browse the contents of the network'), self._app_list)

        self.de.connect_to_server
        if self.de.connect_to_server is not None:
            self._add_app_button(_('Connect to Server'), 'network-server', section, 'raw', self.de.connect_to_server,
                                 _('Connect to a remote computer or shared disk'), self._app_list)
    ...
    def _fill_bookmarked_places_list(self):
        ...
        self._add_app_button(_('Home'), 'user-home', section, 'xdg', self._home_folder_path,
                             _('Open your personal folder'), self._app_list)
        ...
        if self._bookmark_monitor is None:
            self._bookmark_monitor = gio.File(
    ...
    def _fill_favorites_list(self, section, list_name):
        ...
        for app in self.settings[list_name]:
            ...
            app_button = self._add_app_button(app['name'], app['icon name'], section, app['type'], app['command'],
                                              app['tooltip'], self._app_list)
            ...
            if section == self._view.SIDEPANE_SECTION:
                ...
                sidepane_button = self._view.add_sidepane_button(button_str, app['icon name'], self._view.SIDE_PANE,
                                                                 tooltip)
    ...
    def _fill_session_list(self):
        ...
        for item in items:
            app_button = self._add_app_button(item[0], item[2], self._view.SESSION_SECTION, 'raw', item[3], item[1],
                                              self._app_list)
    ...
    def add_section(self, title_str, icon_name=None, tooltip='', hidden_when_no_query=False, node=None,
                    system_menu=False):
        """
    ...
    def _build_reorderable_sections(self):
        ...
        for basename in self.settings['active plugins']:
            ...
            if basename == 'applications':
                ...
                self._view.build_uncategorized_section(_('Uncategorized'),
                                                       _('Items that are not under any menu category'))
                ...
                for category in xrange(plugin_class.category_count):
                    ...
                    plugin.section, dummy = self.add_section(plugin.category_name, plugin.category_icon,
    ...
    def _add_tree_to_app_list(self, tree, section, app_list, recursive=True):
        ...
        for entry in tree:
            ...
            if entry.is_entry():
                ...
                self._add_app_button(entry.get_name(), entry.get_icon(), section, 'app', entry.get_path(),
                                     entry.get_comment(), app_list)
    ...
    def _app_is_valid_folder_or_file(self, app_info):
        ...
        if path_type not in ('computer', 'network', 'trash') and extension != '.desktop':
            ...
            if os.path.isdir(unescaped_path):
                return 1
            elif os.path.isfile(unescaped_path):
                return 2
    ...
    def _untoggle_and_show_all_sections(self):
        ...
        for sec in self._section_list:
            if self._section_list[sec]['must show'] and self._section_list[sec][
                'is system section'] == self._in_system_menu_mode:
